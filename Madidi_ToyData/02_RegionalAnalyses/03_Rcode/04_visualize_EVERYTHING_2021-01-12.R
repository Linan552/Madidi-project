...
for(i in 1:length(datasets))
{
  ...
  {   
    ...
    for(j in 1:length(cutoffs))
    {
      ...
      ##########################################################################
      #createoutput directory if not already done
      subDir <- "04_Results"
      ifelse(!dir.exists(file.path(project.directory, subDir, dataset.i, morpho.opt.m, paste0(cutoff.j, "my"), "Figures")), dir.create(file.path(project.directory, subDir, dataset.i, morpho.opt.m, paste0(cutoff.j, "my"), "Figures")), FALSE)
