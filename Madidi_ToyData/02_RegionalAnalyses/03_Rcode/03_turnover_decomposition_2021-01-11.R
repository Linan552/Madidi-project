...
project.directory <- "~/Madidi_ToyData/02_RegionalAnalyses/"
...
datasets <- c("LargePlots")
morpho.opts <- c("WithMorphos", "NoMorphos")
cutoffs <- c(60, 30)

# set V.phylomaker options for "scenario" (see manual for details)
#phylo.opt <- "S1"
phylo.opt <- "S3"
...
#open tree data
trees.data <- read.csv("02_Data/MergedCleanData/ToyData_trees.txt", 
  header=TRUE, as.is=TRUE, sep = "\t", fileEncoding="UTF-8")

# edit species names in the tree data to include and underscore instead of a space
trees.data$SpeciesName <- gsub('\\s+', '_', trees.data$SpeciesName)


#open merged plot info
plots.data <- read.csv("02_Data/MergedCleanData/ToyData_plots.txt", 
  header=TRUE, as.is=TRUE, sep = "\t", fileEncoding="UTF-8")
...
for(i in 1:length(datasets))
{
  ...
  {   
    ...
    for(j in 1:length(cutoffs))
    {
      ...
      ## create directories and write all output files
      subDir <- "04_Results"
      ifelse(!dir.exists(file.path(project.directory, subDir, dataset.i)), dir.create(file.path(project.directory, subDir, dataset.i)), FALSE)
      ifelse(!dir.exists(file.path(project.directory, subDir, dataset.i, morpho.opt.m)), dir.create(file.path(project.directory, subDir, dataset.i, morpho.opt.m)), FALSE)
      ifelse(!dir.exists(file.path(project.directory, subDir, dataset.i, morpho.opt.m, paste0(cutoff.j, "my"))), dir.create(file.path(project.directory, subDir, dataset.i, morpho.opt.m, paste0(cutoff.j, "my"))), FALSE)
      ...
      save(x=compo.imj, 
           file = paste0(project.directory,"04_Results/", 
                         dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/CompoData.rds"))
      ...
      #change the name of the object because I am terrible and save with new name that will be called in script 04
      BC.total.emp <- BC.empirical
      save(x=BC.total.emp, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/BC.total.emp.rds"))
      
      #change the name of the object because I am terrible and save with new name that will be called in script 04
      S.total.emp <- S.empirical
      save(x=S.total.emp, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/S.total.emp.rds"))
  
  
      save(x=clade.data.null, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/clade.data.null.rds"))    
      
          
      save(x=BC.ag.null, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/BC.ag.null.rds"))
  
      save(x=BC.wg.null, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/BC.wg.null.rds"))
      
      save(x=BC.ws.null, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/BC.ws.null.rds"))
      
      
      save(x=S.ag.null, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/S.ag.null.rds"))
      
      save(x=S.wg.null, 
        file = paste0(project.directory,"04_Results/", 
          dataset.i, "/", morpho.opt.m, "/", cutoff.j, "my/S.wg.null.rds"))    
